...
export declare namespace ProductInterface {
  ...
  interface ProductInfoListFetchFidle
    extends Partial<HTTPInterface.FetchField> {
    barcode?: string; // 条码
    brand?: string; // 品牌
    id?: number; // 商品id
    name?: string; // 商品名称
...
interface ProductInterfaceMap {
  ...
  cashierQueryStatus(
    params: ProductInterface.ProductCashierQueryStatus
  ): string;
...
class ProductInterfaceMap {
  ...
  public productInfoType = (
    params: ProductInterface.ProductInfoTypeFetchFidle
  ) => {
  ...
  public productInfoList = (
    params?: ProductInterface.ProductInfoListFetchFidle
  ) => {
    return `/customer/product/list${params ? jsonToQueryString(params) : ""}`;
  };

  public productInfoDetail = (
    params: ProductInterface.ProductDetailFetchFidle
  ) => {
    return `/customer/product/detail/${params.id}/${BASE_PARAM.institutionCode}`;
  };
}
