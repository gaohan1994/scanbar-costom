...
class Page extends Taro.Component<Props, State> {
  ...
  public createOrder = async () => {
    try {
      ...
      Taro.hideLoading();
      const payment = await productSdk.requestPayment(
  ...
  onTimeClick = (time: string) => {
    ...
    // if (selectTime !== '立即送出') {
    const selectTimeStr = `${
      time === "立即自提" || time === "立即送出" ? "" : currentDate.date || ""
    }${time !== "立即自提" && time !== "立即送出" ? " " : ""}${time}`;
    productSdk.preparePayOrderDetail(
      { planDeliveryTime: selectTimeStr },
      dispatch
    );
    // }
  ...
  getTimeList = () => {
    ...
    if (
      ...
      this.setState({ selectTime: timeList[0] }, () => {
        const selectTimeStr = `${
          timeList[0] === "立即自提" || timeList[0] === "立即送出"
            ? ""
            : currentDate.date || ""
        }${
          timeList[0] !== "立即自提" && timeList[0] !== "立即送出" ? " " : ""
  ...
  getDateList = () => {
    ...
    if (currentDate.id === undefined) {
      this.setState(
        { currentDate: dateList[0], selectDate: dateList[0] },
        () => {
          this.getTimeList();
        }
      );
  ...
  render() {
    ...
    return (
      <View
        ...
        style={{ backgroundColor: "#f2f2f2", height: "115vh" }}
      >
              ...
              items={[
                {
                  ...
                  extraText:
                    payOrderDetail && payOrderDetail.remark
                      ? payOrderDetail.remark
                      : "请输入备注信息",
                  extraTextColor:
                    payOrderDetail && payOrderDetail.remark
                      ? "#333333"
        ...
        <AtFloatLayout
          ...
          className={
            process.env.TARO_ENV === "h5" ? `${cssPrefix}-modal-h5-layout` : ""
          }
          title={`选择${
            payOrderDetail.deliveryType === 0 ? "自提" : "配送"
          }时间`}
          onClose={() => {
        ...
        >
          ...
          >
            <ProductMenu
              ...
              className={
                process.env.TARO_ENV === "h5"
                  ? `${cssPrefix}-modal-h5-menu`
                  : `${cssPrefix}-modal-menu`
              }
            />
            ...
            <ScrollView
              ...
              className={
                process.env.TARO_ENV === "h5"
                  ? `${cssPrefix}-modal-h5-list`
                  : `${cssPrefix}-modal-list`
              }
            >
              {timeList &&
                timeList.length > 0 &&
                  ...
                  return (
                    <View
                      ...
                      className={classnames(
                        process.env.TARO_ENV === "h5"
                          ? `${cssPrefix}-modal-h5-list-item`
                          : `${cssPrefix}-modal-list-item `,
                        {
                          [process.env.TARO_ENV === "h5"
                            ? `${cssPrefix}-modal-h5-list-select`
                            : `${cssPrefix}-modal-list-select`]:
                            selectTime === time
                        }
                      )}
                    >
