...
import { getOrderDetail, getDeliveryFee, getPointConfig, getAbleToUseCouponList } from '../../reducers/app.order';
...
class Page extends Taro.Component<Props, State> {
    ...
    componentDidMount() {
        ...
        orderAction.getPointConfig(this.props.dispatch, currentMerchantDetail);
    }
    componentWillReceiveProps = async (nextProps) => {
        ...
        if (payOrderAddress.id && this.state.isFirst === true) {
            ...
            if (result.code !== ResponseCode.success) {
                productSdk.preparePayOrderAddress({} as any, dispatch);
            }
            this.setState({
                isFirst: false
            })
    ...
    render() {
        const { payOrderProductList, payOrderDetail, activityList, memberInfo, productSDKObj, productCartList, DeliveryFee, pointConfig, couponList } = this.props;
        ...
        const price = payOrderProductList && payOrderProductList.length > 0
            ? numeral(productSdk.getProductTransPrice(activityList, memberInfo, productCartList, payOrderProductList)).format('0.00')
            : '0.00';
