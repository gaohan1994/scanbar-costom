...
class Footer extends Taro.Component<Props> {
  ...
  public renderPrice = () => {
    ...
    return (
      <View>
        <View
          className={`${cssPrefix}-normal component-cart-text`}
          style={process.env.TARO_ENV === "h5" ? { lineHeight: "unset" } : {}}
        >
          ...
          {price && (
            <Text className={`${cssPrefix}-price component-cart-text-price`}>
              {price.split(".")[0]}
            </Text>
          )}
          {price && (
            <Text
              className={`${cssPrefix}-price-bge ${cssPrefix}-price-pos `}
            >{`.${price.split(".")[1]}`}</Text>
          )}
          {/* {priceOrigin && ( 
        ...
        {priceDiscount && (
          <Text className={`${cssPrefix}-price-discount `}>
            {priceDiscount}
          </Text>
        )}
      </View>
    ...
    return (
      <View className={`${prefix}-cart`} style={this.props.style}>
        <View className={`${prefix}-cart-box`}>
          <View className={`${prefix}-cart-left`}>{this.renderPrice()}</View>
          {this.renderStepper()}
        </View>
      </View>
    );
