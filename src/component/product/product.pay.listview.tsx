...
import { getPayOrderDetail, getProductCartList } from '../../common/sdk/product/product.sdk.reducer';
...
class ProductPayListView extends Taro.Component<Props, State> {
  ...
  render() {
    const { productList, className, padding, payOrderDetail, type, orderDetail, showCallModal, DeliveryFee, isDetail } = this.props;
    ...
    return (
      ...
      >
        ...
        >
              ...
              <View className={`${cssPrefix}-row-totals`}>
                <View className={`${cssPrefix}-row-content-item`}>
                  ...
                  <View className={`${cssPrefix}-row-voucher-wrap`}>
                    ...
                    >
                      ￥{numeral(isDetail ? order.deliveryFee : DeliveryFee).format('0.00')}
                    </Text>
                    {/* <Image
          ...
          {isDetail && this.renderDetailDiscount()}
          {!isDetail && this.renderPayDiscount()}
          {isDetail && this.renderDetailTotal()}
          {!isDetail && this.renderPayTotal()}
        </View>
      </View>
    );
  }
  ...
  renderPayDiscount = () => {
    const { payOrderDetail, productCartList, memberInfo, pointConfig, activityList, dispatch, DeliveryFee } = this.props;
    ...
    const PointsPre = pointConfig && pointConfig.deductRate ? numeral(numeral(memberInfo.points * pointConfig.deductRate < pointPrice ? memberInfo.points * pointConfig.deductRate : pointPrice).format('0.00')).value() : 0;
    const MathPointsPre = pointConfig && pointConfig.deductRate ? Math.ceil(PointsPre / pointConfig.deductRate) : 0;
    return (
      <View>
          ...
          <View className={`${cssPrefix}-row-totals`}>
            <View className={`${cssPrefix}-row-content-item ${cssPrefix}-row-content-column`}>
              <View className={`${cssPrefix}-row-content-column-item`}>
                ...
                <Text className={`${cssPrefix}-row-content-price`}>{`-￥${totalActivityMoney}`}</Text>
              </View>
            </View>
          </View>
        )}
        ...
        {
          memberInfo && memberInfo.points ? (
            <View className={`${cssPrefix}-row-totals`} >
              <View className={`${cssPrefix}-row-content-item`}>
                <Text className={`${cssPrefix}-row-voucher`}>积分抵现<Text className={`${cssPrefix}-row-voucher-span`}>({MathPointsPre || 0}积分)</Text></Text>
                <View className={`${cssPrefix}-row-content-row`}>
                  <Text className={`${cssPrefix}-row-content-price`}>
                    {`-￥${PointsPre || 0}`}
                  </Text>
      ...
      <View>
          ...
          <View className={`${cssPrefix}-row-totals`}>
            <View className={`${cssPrefix}-row-content-item ${cssPrefix}-row-content-column`}>
              <View className={`${cssPrefix}-row-content-column-item`}>
                ...
                <Text className={`${cssPrefix}-row-content-price`}>{`-￥${orderActivityInfoListTotal}`}</Text>
              </View>
            </View>
          </View>
        )}
        ...
        }
        {
          order && order.pointDiscount ? (
            <View className={`${cssPrefix}-row-totals`} >
              <View className={`${cssPrefix}-row-content-item`}>
                <Text className={`${cssPrefix}-row-voucher`}>积分抵现<span className={`${cssPrefix}-row-voucher-span`}>({Math.ceil(order.pointDiscount || 0)}积分)</span></Text>
                <View className={`${cssPrefix}-row-content-row`}>
                  <Text className={`${cssPrefix}-row-content-price`}>
  ...
  private renderPayTotal = () => {
    const { memberInfo, pointConfig, DeliveryFee, payOrderDetail, payOrderProductList, productSDKObj, productCartList, activityList } = this.props;
    const { pointSet } = this.state;
  ...
  renderDetailTotal = () => {
    ...
    const { order } = orderDetail;
    const discountPriceFoot = order.totalAmount - order.transAmount;
    const transAmountString = numeral(order.transAmount).format('0.00');
      ...
      <View className={`${cssPrefix}-row-totals`}>
        <View className={`${cssPrefix}-row-content-item`}>
          ...
          <View className={`${cssPrefix}-row-tran`}>
            ...
            <Text className={`${cssPrefix}-row-tran ${cssPrefix}-row-tran-margin`}>{`合计：`}</Text>
            <Text className={`${cssPrefix}-row-tran-price`}>￥</Text>
            <Text className={`${cssPrefix}-row-tran-price ${cssPrefix}-row-tran-big `}>{transAmountString.split('.')[0]}</Text>
            <Text className={`${cssPrefix}-row-tran-price`}>{`.${transAmountString.split('.')[1]}`}</Text>
          </View>
        </View>
      </View>
    )
